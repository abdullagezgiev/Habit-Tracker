...
//       newHabit.notificationId = await scheduleHabitNotification(newHabit) as string;
...
//         <AddHabitForm
//           onAdd={async (title) => {
//             const time = '10:00'; // или получать из формы
//             await addHabit(title, time);
//           }}
...
const HabitTracker = () => {
  ...
  const [habits, setHabits] = useState<Array<{
    title: string;
    time: string | undefined;id: string, 
    name?: string, notificationId: string | null
}>>([]);
  const [isAdding, setIsAdding] = useState(false);
  const [, setHabitToDelete] = useState<string | null>(null);
  const [newHabitTime] = useState('10:00');
